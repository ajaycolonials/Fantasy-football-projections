...
def upsert_projections_ppr(proj_df: pd.DataFrame):
    ...
    NOTE:
    - Your projections.format enum is ('ppr','half_ppr','standard') so we use 'ppr'.
...
# Add nonlinear age feature
    ...
    # (We also drop games_played to avoid "durability leakage" into PPG skill model.)
    exclude = {
    "player_id",
    "season",
    "fantasy_points_ppr",
    ...
    # Strongly recommended for a "healthy PPG" model:
    ...
    for pos in POSITIONS:
        ...
        # Just for interpretability: what does this mean in a 17-game season?
    ...
    for pos, model in final_models.items():
        ...
        X_2025.columns = [f"{c}_x" for c in features]  # match training column names
