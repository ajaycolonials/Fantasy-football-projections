# rf_train_and_project_2026_holdout_ppg17_teamcontext.py
...
def main():
    ...
    exclude = {
        "player_id",
        "season",
        "team_id",              # important: do not use raw ids as features
        "fantasy_points_ppr",
        "fantasy_points_half",
        "fantasy_points_std",
        "pass_attempts",
    ...
    print("[INFO] points_per_game in features:", "points_per_game" in features)
    print("[INFO] plays_per_game in features:", "plays_per_game" in features)
    print("[INFO] pass_rate_neutral_script in features:", "pass_rate_neutral_script" in features)
    ...
    for pos in POSITIONS:
        ...
        print(
            pos_all.sort_values("error", ascending=False)
            .head(10)
            .to_string(index=False)
        )
        ...
        print(
            f"[ROLLING VAL] {pos}  "
            f"MAE_PPG={mae:.2f}  MAE_17G_PTS={mae_pts_17:.2f}"
        )

        all_results.append(pos_all)
        holdout_models[pos] = True    
